...
- [x] ~~Boot Windows — confirm mouse connects automatically without re-pairing~~
  Still failed — see Session 8.

---

## Session 8 — BLE Sync Direction Reversal (Windows → Linux)

### Problem

After patching LTK, IRK, CSRKs, EDIV, and ERand into the correct active Windows
registry entry, the mouse **still** did not connect in Windows. Power cycling,
disconnecting in Linux before reboot, and deleting stale registry entries all failed.

### Root Cause: BLE Host Identity

BLE devices store the **host's adapter IRK** during pairing. The mouse recognises its
host by this IRK, not by MAC address. Linux and Windows have **different adapter IRKs**:

- Linux adapter IRK: unknown (generated by BlueZ kernel, not easily accessible)
- Windows `CentralIRK`: `55D9B350A14AC2E67EFCEDDDBFF5D7AF`

When the mouse is paired in Linux, it stores Linux's adapter IRK. When Windows tries
to connect (even with correct device keys), the mouse rejects it because Windows
presents a different host identity.

This is a fundamental BLE asymmetry — unlike Classic BR/EDR where link keys are
symmetric, BLE bonds are directional.

### Solution: Reverse BLE Sync Direction

Every working dual-boot BLE guide copies keys **Windows → Linux**, not the reverse:

1. Pair in Windows (last) — mouse stores Windows' host identity
2. Copy Windows device keys into Linux's BlueZ info file
3. `systemctl restart bluetooth` — Linux now uses Windows' keys
4. Mouse recognises both OSes (both present Windows' keys)

### Changes Made

- **`linux_bt.py`**: Added `patch_ble_info_file()` — writes Windows keys (byte-reversed)
  into a BlueZ info file, preserving `[General]` and metadata sections. Uses
  `_write_info_file()` with a casing map (`_BLUEZ_KEY_CASING`) to preserve BlueZ's
  expected key casing (configparser lowercases everything by default).

- **`sync.py`**: Reversed BLE sync direction:
  - `_sync_ble()` now reads the matched Windows entry and writes its keys into the
    Linux info file (was: read Linux, patch Windows)
  - Added `_win_key_to_linux_hex()` helper
  - Removed `_mac_to_address_int()` (no longer needed)
  - Removed `create_ble_entry` and `patch_ble_entry` imports
  - `_needs_any_write()` → `_needs_classic_write()` (BLE no longer touches the hive)
  - Final output now says "Run: sudo systemctl restart bluetooth"
  - Module docstring and CLI description updated

- Classic BR/EDR sync direction **unchanged** (Linux → Windows) — symmetric link keys
  work fine in either direction.

- **Tests**: 3 new tests for `patch_ble_info_file`:
  - Updates all key fields correctly
  - Preserves `[General]` metadata
  - Creates missing sections from scratch

### Test Results

26 tests passing. Linting and formatting clean.
